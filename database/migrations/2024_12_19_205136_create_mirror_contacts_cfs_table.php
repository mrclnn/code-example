...
class CreateMirrorContactsCfsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('mirror_contacts_cfs', function (Blueprint $table) {
            $table->id();

            $table->integer('cf_id')->index();
            $table->string('cf_code')->nullable()->index();
            $table->unsignedBigInteger('entity_id')->index();
            $table->foreign('entity_id')->references('id')->on('mirror_contacts')->cascadeOnUpdate()->cascadeOnDelete();
    ...
    {
        Schema::dropIfExists('mirror_contacts_cfs');
    }
}
