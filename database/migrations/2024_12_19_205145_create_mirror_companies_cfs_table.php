...
class CreateMirrorCompaniesCfsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('mirror_companies_cfs', function (Blueprint $table) {
            $table->id();

            $table->integer('cf_id')->index();
            $table->string('cf_code')->nullable()->index();
            $table->unsignedBigInteger('entity_id')->index();
            $table->foreign('entity_id')->references('id')->on('mirror_companies')->cascadeOnUpdate()->cascadeOnDelete();
    ...
    {
        Schema::dropIfExists('mirror_companies_cfs');
    }
}
